...
public class MusicEvents
{
	...
	{
		if (event.track.getUserData() instanceof TrackData) {
			TrackData data = (TrackData)event.track.getUserData();
		...
		if (event.track.getUserData() instanceof TrackData) {
			...
			if (data.channel != null) {
				...
				if (MusicCommand.musicChannel.containsKey(data.guild.getIdLong())) {
					TextChannel channel1 = Startup.discordClient.getTextChannelById(MusicCommand.musicChannel.get(data.guild.getIdLong()));
	...
	public static void buttonEvent(ButtonInteractionEvent event){
		if(event.getButton().getId().startsWith("skip-")){
			...
			if(getGuildAudioPlayer(event.getGuild()).currentTrack != null) {
				if (getGuildAudioPlayer(event.getGuild()).currentTrack.getUserData() instanceof TrackData) {
					TrackData data = (TrackData)getGuildAudioPlayer(event.getGuild()).currentTrack.getUserData();
					
					if(data.url.equals(url)){
						Startup.discordClient.getEventManager().handle(new SkipTrackEvent(event.getGuild().getJDA(), getGuildAudioPlayer(event.getGuild()).getPlayer().getPlayingTrack()));
			...
			slashBuilder.addAction(ComponentResponseSystem.addComponent(menuID1, event.getUser(), event, menuBuilder1.build(), (e2) -> {
				...
				for(Track tr : object1.tracks) {
					try {
						track = LavalinkUtil.toAudioTrack(tr.track);
	...
	public static void handle(EndTrackEvent event)
	{
		...
		if (event.newTrack == null) {
			if (event.oldTrack.getUserData() instanceof TrackData) {
				TrackData data = (TrackData)event.oldTrack.getUserData();

...
@SubCommand( parent = HistorySlashCommand.class )
public class History extends MusicCommand
{
	...
	public void slashCommandExecuted(SlashCommandInteractionEvent slashEvent, Guild guild, User author, SlashCommandChannel channel, SlashCommandMessage message)
	{
		...
		builder.setTitle(
				"**Show last " + (objects.size() < maxHistory ? objects.size() : maxHistory) + " songs played**");
	...
	{
		@SlashArgument( key = "remove", text = "Which tracks you want to remove from your history", required = true )

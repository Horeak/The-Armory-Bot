...
{
	@SlashArgument( key = "page", text = "Which page of the queue you want to view." )
	public int page = 0;
	...
	{
		ArrayList<QueueObject> tracks = new ArrayList<QueueObject>();
		...
		if (MusicCommand.getGuildAudioPlayer(guild).getPlayer().getPlayingTrack() != null) {
			...
			builder1.addField("**Currently playing**",
			                  "[**" + name + "**](" + track.getInfo().uri + ")" + space + "(" + time + ")", false);
		}
		...
		builder1.addField("**Queue time**", "**" + (tTime != null && !tTime.isEmpty() ? tTime : "N/A") + "**", true);

...
public class ViewRemindersCommand  implements ISlashCommand
{
	...
	{
		ArrayList<Entry<UUID, remindObject>> entList = new ArrayList<>(ReminderCommand.objects.entrySet());
		...
		if(entList.size() > 0){
			...
			SelectMenu.Builder menuBuilder = SelectMenu.create(selectionId.toString()).setPlaceholder("Choose reminder");
			...
			SelectMenu menu = (SelectMenu)ComponentResponseSystem.addComponent(selectionId, author, (channel instanceof SlashCommandChannel ? channel.event : null), menuBuilder.build(), (e) -> {
				String id = ((SelectMenuInteractionEvent)e).getValues().get(0);
				...
				if(id != null){
					...
					if(ob != null){
						...
						Button cancel = (Button)ComponentResponseSystem.addComponent(author, Button.secondary("id", "Cancel reminder"), (ev) -> {
							remindObject object = ReminderCommand.objects.remove(UUID.fromString(id));

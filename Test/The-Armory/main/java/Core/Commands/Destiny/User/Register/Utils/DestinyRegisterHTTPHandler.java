...
public class DestinyRegisterHTTPHandler implements HttpHandler
{
	...
	public void handle(HttpExchange t) throws IOException
	{
		...
		if (code == null) {
			response = Destiny2RegisterCommand.getHTML("There was an error authorizing from bungie!",
			                                           "There may be an issue with Bungies servers.",
		...
		if (code != null && ent != null) {
			...
			if (userObject != null) {
				...
				if (accessToken != null) {
					...
					Destiny2RegisterCommand.finalMessage(ent, dUsername, accounts, userObject.getAsTag());

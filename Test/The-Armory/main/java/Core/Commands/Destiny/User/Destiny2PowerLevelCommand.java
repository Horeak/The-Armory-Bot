...
import Core.Commands.Destiny.DestinyDataBaseObjects.Destiny2.Destiny2ItemBucket;
...
public class Destiny2PowerLevelCommand implements ISlashCommand
{
					...
					for (String key : data.keySet()) {
						...
						if (ob1 instanceof JSONObject) {
							...
							btns.add(ComponentResponseSystem.addComponent(author, Button.secondary("id", race + " " + clas), (event) -> {
								...
								if(profileObject.has("characterInventories")){
									JSONObject tempObject = profileObject.getJSONObject("characterInventories");
									checkItems(profileObject, charName, data, classType, items, powerItems, tempObject, "characterInventories");
								}
								
								//And finaly check equipped items from all classses
								if(profileObject.has("characterEquipment")){
									JSONObject tempObject = profileObject.getJSONObject("characterEquipment");
									checkItems(profileObject, charName, data, classType, items, powerItems, tempObject, "characterEquipment");
								}
								...
								for(Entry<Long, CustomEntry<Integer, JSONObject>> ent : powerItems.entrySet()){
									if(Destiny2ItemSystem.destinyBucketObjects.containsKey(ent.getKey().intValue())){
										Destiny2ItemBucket bucket = Destiny2ItemSystem.destinyBucketObjects.get(ent.getKey().intValue());
								...
								g2.drawString(nextPower + "", scale * 100 + (8 * 110) + 25, scale * 50 + 145);
								...
								GradientPaint gradient = new GradientPaint(0, ((width / emblem.getWidth()) * emblem.getHeight()), avgColor1, x2, y2, avgColor2, true);
								...
								for(Entry<Integer, CustomEntry<Integer, JSONObject>> ent : map.entrySet()){
									...
									Destiny2ItemObject item = Destiny2ItemSystem.destinyItemObjects.get(itemHash.intValue());
									...
									if(item.displayProperties != null && item.displayProperties.hasIcon) {
										try {
											...
											ta.setBounds(xs, ys, xsSize - 20, ysSize);
											ta.setForeground(Color.white);
											ta.setBackground(new Color(0,0,0,0));
											ta.setFont(g2.getFont());
											
											Graphics g3 = g2.create(xs, ys, xsSize, ysSize);
											ta.paint(g3);
											...
											if(!isArtifact) {
												if (val.getValue().has("item_location")) {
													g3 = g2.create(xs, ys + (ysSize - 50), xsSize, 50);
																...
																if (item.has("bucketHash") && item.has("itemInstanceId")) {
																	...
																	if (itemBucket.equals(bucketHash)) {
																		if (refId != item.getLong("itemInstanceId")) {
																			...
																			Destiny2ItemObject itemObj = Destiny2ItemSystem.destinyItemObjects.get(itemHash.intValue());
	...
	private void checkItems(JSONObject profileObject,
			String charName, JSONObject characters, int classType, HashMap<Long, JSONObject> items, HashMap<Long, CustomEntry<Integer, JSONObject>> powerItems,JSONObject tempObject, String type)
	{
		if (tempObject.has("data")) {
			JSONObject itemsData = tempObject.getJSONObject("data");
						...
						if (instanceObject != null && instanceObject.has(
							...
							if (stat.has("statHash")){
								...
								if(statHash == 1480404414L || statHash == 1935470627L || statHash == 3897883278L || statHash == 3289069874L){
									...
									Destiny2ItemObject itemObject1 = Destiny2ItemSystem.destinyItemObjects.get(itemHash.intValue());
									...
									if (itemObject != null) {
										if (itemObject1.classType == 3 || itemObject1.classType == classType) {
											...
											if(type.equals("profileInventory")){
												itemObject.put("item_location", "Located in Vault");
												
											}else if(type.equals("characterEquipment")){
												itemObject.put("item_location", "Equipped on " + charName);
												itemObject.put("location_id", charId);
											}else if(type.equals("characterInventories")){
												...
												if(plug.has(id.toString())){
													...
													for(Object sock : plugsArray){
														...
														if(socket.has("plugHash")){
															...
															Destiny2ItemObject obj = Destiny2ItemSystem.destinyItemObjects.get(plugHash.intValue());
